/* OrbStage — the persistent HatchArtifact above the route tree.
 *
 * The artifact is rendered once at the top of the layout (z above Atmosphere,
 * z below interactive content). Routes don't render their own artifact; instead,
 ...
 * reads its bounding rect, and animates the artifact to fill it.
 *
 * On route change, the artifact morphs both POSITION (rect) and STATE (sealed →
 * incubating → hatching → public) via FM spring + the artifact's internal
 * interpolation. There is one continuous artifact across the whole app.
 *
 * Anchor element protocol:
 *   <div data-orb-anchor data-orb-state="incubating" />
 *   <div data-orb-anchor data-orb-progress="0.35" />    ← scroll-driven
 *
...
import { HatchArtifact } from "./hatch-artifact.js";
...
const DEFAULT_TARGET: OrbTarget = {
  left: -9999, top: -9999, width: 0, height: 0, state: "incubating", progress: null,
};

export function OrbStage() {
  const [target, setTarget] = useState<OrbTarget>(DEFAULT_TARGET);
  const [hasSnappedInitialTarget, setHasSnappedInitialTarget] = useState(false);
  ...
  useEffect(() => {
    const measure = () => {
      ...
      if (!el) {
        ...
        // static logo mark instead of the animated artifact). Reset to the
        // off-screen sentinel so the artifact doesn't hover at its last position.
      ...
      const progressAttr = el.getAttribute("data-orb-progress");
      const progress = progressAttr !== null ? parseFloat(progressAttr) : null;
      setTarget({ left: r.left, top: r.top, width: r.width, height: r.height, state, progress });
    };
    ...
    mo.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ["data-orb-anchor", "data-orb-state", "data-orb-progress"] });
  ...
  useEffect(() => {
    if (!visible || hasSnappedInitialTarget) return;
    const id = requestAnimationFrame(() => setHasSnappedInitialTarget(true));
    return () => cancelAnimationFrame(id);
  }, [hasSnappedInitialTarget, visible]);

  const stageTransition = hasSnappedInitialTarget ? {
